<?php
/**
 * Lucide icon helper — returns inline SVG markup.
 * All paths sourced from lucide.dev (MIT licence).
 * SVG inherits color via stroke="currentColor", so icons match
 * surrounding text on both light and dark backgrounds.
 *
 * Usage: echo icon('plus', 16);
 *        echo icon('arrow-left', 14, 'some-extra-class');
 */
function icon(string $name, int $size = 16, string $class = ''): string {
    ...
    $inner = $icons[$name] ?? $icons['circle-alert'];
    $cls   = trim('icon ' . $class);
